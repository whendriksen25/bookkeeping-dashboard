...
export default async function handler(req, res) {
  ...
  try {
    ...
    const totalCogs = pl.cogs ?? 0;
    const operatingExpenses = pl.expenses ?? 0;
    const otherIncome = pl.otherIncome ?? 0;
    const otherExpense = pl.otherExpense ?? 0;
    const net = pl.net ?? revenue - (totalCogs + operatingExpenses + otherExpense);
    ...
    const rows = [
      ...
      [],
      ["Metric", "Amount"],
      ["Revenue", formatAmount(revenue, currency)],
      ["Cost of goods sold", formatAmount(-Math.abs(totalCogs), currency)],
      ["Gross profit", formatAmount(grossProfit, currency)],
      ["Operating expenses", formatAmount(-Math.abs(operatingExpenses), currency)],
      ["Other income", formatAmount(otherIncome, currency)],
      ["Other expenses", formatAmount(-Math.abs(otherExpense), currency)],
      ["Net profit", formatAmount(net, currency)],
    ];

    const csv = toCsv(rows);
    const filename = normalizedProfile
      ? `profit-and-loss-${normalizedProfile}.csv`
      : "profit-and-loss-overall.csv";

    res.setHeader("Content-Type", "text/csv; charset=utf-8");
    res.setHeader("Content-Disposition", `attachment; filename=${filename}`);
    res.status(200).send(csv);
  } catch (err) {
    console.error("[reports] profit and loss export failed", err);
