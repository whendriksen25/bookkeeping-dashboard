  ...
  try {
    ...
    const { rows: upsertRows } = await client.query(
      `INSERT INTO invoices (
        id, invoice_number, invoice_date, vendor_name, payment_method,
    ...
    await client.query(`DELETE FROM invoice_items WHERE invoice_id = $1`, [effectiveInvoiceId]);

    if (Array.isArray(lineItems)) {
      for (const item of lineItems) {
        const itemId = crypto.randomUUID();
        const profileId = item.profileId ? Number(item.profileId) : null;

        await client.query(
          `INSERT INTO invoice_items (
            id, invoice_id, line_index, description, normalized_name, quantity, unit,
