...
async function upsertVendorFromInvoice({ userId, factuurdetails, invoiceId, invoiceTotalIncl, invoiceCurrency }) {
  ...
  const existing = await pool.query(
    `SELECT id, display_name FROM vendors WHERE user_id = $1 AND (
        ($2 IS NOT NULL AND vendor_key = $2)
        OR (vendor_key IS NULL AND display_name = $3)
      )
      ORDER BY vendor_key IS NULL, id
      LIMIT 1`,
    [userId, vendorKey, trimmedName]
  );
